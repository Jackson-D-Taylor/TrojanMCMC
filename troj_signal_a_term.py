...
@enterprise_function
def troj_res_targeted_theta_low(toas, n_b, log_B, nu, log_eta, phi, T_asc):
    """
    Calculate the timing perturbation due to an object passing a pulsar
    on a hyperbolic orbit.
    ...
    log_B : log10 of libration amplitude [lts]
    nu   : libration frequency [rad s^-1]
    log_eta  : theta - pi/3 [rad]
    phi  : arbitrary phase for the n_b - nu frequency term
...
def troj_res_block_targeted_theta_low(
    ...
    T_asc,
    log_B=Uniform(-12, -4),
    log_eta=Uniform(np.log10(7 * 10**-7), np.log10(pi / 3)),
    phi=Uniform(0, 2 * pi),
    name="troj",
):

    return Deterministic(
        troj_res_targeted_theta_low(
            log_B=log_B, nu=nu, log_eta=log_eta, phi=phi, n_b=n_b, T_asc=T_asc
        ),
        name=name,
    )
